...
static TX_THREAD * tcp_client_thread_handle;
...
static unsigned char tcp_client_stack[THREAD_STACK_SIZE];
...
void quec_tcp_client_thread(ULONG param)
{
	int ret = 0;
	ULONG   dss_event = 0;
	int32   sig_mask;
...
#ifdef QAPI_TXM_MODULE
	if (TX_SUCCESS != txm_module_object_allocate((VOID *)&tcp_client_thread_handle, sizeof(TX_THREAD))) 
	{
		return -1;
	}
#endif
	ret = tx_thread_create(tcp_client_thread_handle, "TCP Client Thread", quec_tcp_client_thread, NULL,
							tcp_client_stack, THREAD_STACK_SIZE, THREAD_PRIORITY, THREAD_PRIORITY, TX_NO_TIME_SLICE, TX_AUTO_START);
	if (ret != TX_SUCCESS)
	{
		IOT_INFO("Thread creation failed\n");
		return ret;
