...
static TX_THREAD* mqtt_thread_handle;

static unsigned char tcp_dss_stack[THREAD_STACK_SIZE];

static unsigned char mqtt_stack[THREAD_STACK_SIZE];
...
void quec_mqtt_thread(ULONG param)
{
	int     ret = 0;
	ULONG   dss_event = 0;
	int32   sig_mask;
...
#ifdef QAPI_TXM_MODULE
	if (TX_SUCCESS != txm_module_object_allocate((VOID *)&mqtt_thread_handle, sizeof(TX_THREAD))) 
	{
		return -1;
	}
#endif
	ret = tx_thread_create(mqtt_thread_handle, "MQTT Thread", quec_mqtt_thread, NULL,
							mqtt_stack, THREAD_STACK_SIZE, THREAD_PRIORITY, THREAD_PRIORITY, TX_NO_TIME_SLICE, TX_AUTO_START);
	if (ret != TX_SUCCESS)
	{
