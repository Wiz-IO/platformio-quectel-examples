...
static TX_THREAD* http_thread_handle;

static unsigned char http_client_stack[THREAD_STACK_SIZE];
...
/*===========================================================================
                               FUNCTION
===========================================================================*/
static void http_uart_rx_cb(uint32_t num_bytes, void *cb_data)
{
	if(num_bytes == 0)
	{
...
static void http_uart_tx_cb(uint32_t num_bytes, void *cb_data)
{
	IOT_DEBUG("QT# uart_tx_cb, send [%d]", num_bytes);
}
	
void http_uart_dbg_init()
{
	...
	uart_cfg.rx_CB_ISR			= (qapi_UART_Callback_Fn_t)&http_uart_rx_cb;
	uart_cfg.tx_CB_ISR			= (qapi_UART_Callback_Fn_t)&http_uart_tx_cb;
...
void http_client_cb(void* arg, int32 state, void* http_resp)
{
	...
	HTTP_UART_DBG("http_client_cb:%x,%ld,len:%lu,code:%lu,%x\n",arg,state,resp->length,resp->resp_Code,resp->data);
	if( resp->resp_Code >= 200 && resp->resp_Code < 300)
	{
		if(resp->data != NULL && state >= 0)
		{
			...
			if(file_opened == FALSE)
			{
				...
				if(ret_val == 0)
				{
				HTTP_UART_DBG("open %s success\n",FILE_PATH);
				file_opened = TRUE;
				}  
				else
				{
				HTTP_UART_DBG("open failed:%d\n",ret_val);
				}
			}
			if (file_opened == TRUE)
			{
		...
		else
		{
			...
			{
				HTTP_UART_DBG("download timeout, try again - %d\n", http_session_policy.data_retry);
	...
	else if(resp->resp_Code == 0)
	{
		if (http_session_policy.data_retry++ < HTTP_MAX_DATA_RETRY)
		{
			HTTP_UART_DBG("rsp code = 0, try again - %d\n", http_session_policy.data_retry);
			tx_event_flags_set(http_release_handle, RE_DOWNLOAD_FLAG, TX_OR);
		}
		else
		{
...
static int http_download(char* host, uint16 port, uint8* file_name)
{
	...
	if (ssl_ctx_id)
	{
		...
		* is not necessary. (refer to TX3.0.1 P8101-35 Doc) */
		...
		if(ret != QAPI_OK)
		{
			HTTP_UART_DBG("Start HTTPS connection, SSL config ret[%d]\n", ret);
			return -2;
		}
	}
	else
	...
	do
	{
		...
		{
			reconnect_count++;
			HTTP_UART_DBG("qapi_Net_HTTPc_Connect ERROR ret:%d, re-connect times:%d\n",ret,reconnect_count);
		...
		{
			HTTP_UART_DBG("qapi_Net_HTTPc_Connect success :%d\n", ret);
			break;
		}
	} while (1);
...
int http_download_process(qapi_Net_HTTPc_handle_t http_handler, char *host, uint16 port, char *file)
{
    ...
    do
    {
		...
		HTTP_UART_DBG("http_download: %s\n", range_buffer);
		
		ret = qapi_Net_HTTPc_Add_Header_Field(http_handler, "Range", range_buffer);
		if (ret != QAPI_OK)
		{
			HTTP_UART_DBG("qapi_Net_HTTPc_Add_Header_Field content-type ERROR :%d\n", ret);
            return -2;
		}
	    ...
	    {
	        reconn_counter++;
			HTTP_UART_DBG("qapi_Net_HTTPc_Connect ERROR :%d,reconn_counter:%d\n",ret, reconn_counter);
		...
		HTTP_UART_DBG("qapi_Net_HTTPc_Connect re-content times:%d\n", reconn_counter);
    } while (1);
...
void qt_http_init_device_info(void)
{
	...
	if(status != QAPI_OK)
	{
		HTTP_UART_DBG( "~ qapi_Device_Info_Init fail [%d]\n", status);
	}
	else
	{
		HTTP_UART_DBG( "~ qapi_Device_Info_Init OK [%d]\n", status);
	}
}
...
void quec_http_client_thread(ULONG param)
{
	...
	HTTP_UART_DBG("host:%s,port:%d,source:%s\n", host, port, file_name);
...
int quectel_task_entry(void)
{
	...
	if (TX_SUCCESS != txm_module_object_allocate((VOID *)&http_thread_handle, sizeof(TX_THREAD))) 
	{
		return -1;
	}
#endif
	ret = tx_thread_create(http_thread_handle, "HTTP Thread", quec_http_client_thread, NULL,
							http_client_stack, THREAD_STACK_SIZE, THREAD_PRIORITY, THREAD_PRIORITY, TX_NO_TIME_SLICE, TX_AUTO_START);
	if (ret != TX_SUCCESS)
	{
