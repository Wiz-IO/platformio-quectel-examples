...
int quectel_task_entry(void)
{
   ...
   tx_byte_allocate(byte_pool_gpio, (VOID *)& wbuf_reg,1, TX_NO_WAIT);
    ...
    res = qapi_I2CM_Open(QT_QAPI_I2CM_PORT_01, &client_handle); 
   ...
   qapi_Timer_Sleep(500, QAPI_TIMER_UNIT_MSEC, true); // delay must not be omitted
   qt_uart_dbg(uart_conf.hdlr,"i2c_read:byte1[0x%x], byte2[0x%x],%d\r\n", rbuf[0], rbuf[1],res);
