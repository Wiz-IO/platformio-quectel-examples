...
UCHAR free_memory_time[UART_BYTE_POOL_SIZE];


TX_BYTE_POOL *byte_pool_time_task;
#define TIME_THREAD_PRIORITY   	180
#define TIME_THREAD_STACK_SIZE 	(1024 * 16)
TX_THREAD* TIME_thread_handle; 
char *TIME_thread_stack = NULL;
...
int quectel_time_task_entry(void)
{
...
//	qapi_Timer_Sleep(20, QAPI_TIMER_UNIT_SEC, true);
	...
	ret = tx_byte_pool_create(byte_pool_uart, "time task pool", free_memory_uart, UART_BYTE_POOL_SIZE);
	if(ret != TX_SUCCESS)
	{
...
int quectel_task_entry(void)
{
	...
	ret = txm_module_object_allocate(&byte_pool_time_task, sizeof(TX_BYTE_POOL));
	if(ret != TX_SUCCESS)
	{
		IOT_DEBUG("txm_module_object_allocate [byte_pool_sensor] failed, %d", ret);
		return ret;
	...
	ret = tx_byte_pool_create(byte_pool_time_task, "Sensor application pool", free_memory_time, UART_BYTE_POOL_SIZE);
	if(ret != TX_SUCCESS)
	{
		IOT_DEBUG("tx_byte_pool_create [byte_pool_sensor] failed, %d", ret);
		return ret;
	...
	ret = tx_byte_allocate(byte_pool_time_task, (VOID **) &TIME_thread_stack, TIME_THREAD_STACK_SIZE, TX_NO_WAIT);
	if(ret != TX_SUCCESS)
	{
		IOT_DEBUG("tx_byte_pool_create [byte_pool_sensor] failed, %d", ret);
		return ret;
	...
	ret = txm_module_object_allocate((VOID *)&TIME_thread_handle, sizeof(TX_THREAD));
	if(ret != TX_SUCCESS)
	{
		IOT_DEBUG("txm_module_object_allocate [byte_pool_sensor] failed, %d", ret);
		return ret;
	...
	ret = tx_thread_create(TIME_thread_handle,
						   "TIME Task Thread",
						   quectel_time_task_entry,
						   NULL,
						   TIME_thread_stack,
