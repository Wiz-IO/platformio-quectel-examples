...
UCHAR free_memory_timer[UART_BYTE_POOL_SIZE];

TX_BYTE_POOL *byte_pool_timer_task;
#define TIMER_THREAD_PRIORITY   	180
#define TIMER_THREAD_STACK_SIZE 	(1024 * 16)
TX_THREAD* TIMER_thread_handle; 
char *TIMER_thread_stack = NULL;
...
int quectel_timer_task_entry(void)
{
	...
	ret = tx_byte_pool_create(byte_pool_uart, "timer task pool", free_memory_uart, UART_BYTE_POOL_SIZE);
	if(ret != TX_SUCCESS)
	{
...
int quectel_task_entry(void)
{
	...
	ret = txm_module_object_allocate(&byte_pool_timer_task, sizeof(TX_BYTE_POOL));
	if(ret != TX_SUCCESS)
	{
		IOT_DEBUG("txm_module_object_allocate [byte_pool_sensor] failed, %d", ret);
		return ret;
	...
	ret = tx_byte_pool_create(byte_pool_timer_task, "Sensor application pool", free_memory_timer, UART_BYTE_POOL_SIZE);
	if(ret != TX_SUCCESS)
	{
		IOT_DEBUG("tx_byte_pool_create [byte_pool_sensor] failed, %d", ret);
		return ret;
	...
	ret = tx_byte_allocate(byte_pool_timer_task, (VOID *)&TIMER_thread_stack, TIMER_THREAD_STACK_SIZE, TX_NO_WAIT);
	if(ret != TX_SUCCESS)
	{
		IOT_DEBUG("tx_byte_allocate [rx_buff] failed, %d", ret);
		return ret;
	...
	if(TX_SUCCESS != txm_module_object_allocate((VOID *)&TIMER_thread_handle, sizeof(TX_THREAD))) 
	{
		IOT_INFO("[task] txm_module_object_allocate qt_sub1_thread_handle failed");
		return - 1;
	}

	/* create a new task : quectel_timer_task_entry */
	ret = tx_thread_create(TIMER_thread_handle,
						   "TIMER Task Thread",
						   quectel_timer_task_entry,
						   NULL,
						   TIMER_thread_stack,
