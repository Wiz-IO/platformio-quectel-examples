...
extern void tcp_uart_debug_print(const char* fmt, ...) ;
...
//static const char* connectionString = "HostName=IoTLabDemoCommonIoTHub.azure-devices.net;DeviceId=9206_device;SharedAccessKey=3BSCEX9qhhPCxuSMqWXIoRF0wxdhXUrZpd/QOqx5gZE=";
static const char* connectionString = "HostName=aics-prod-hub.azure-devices.net;DeviceId=S2vSt1rpJM;SharedAccessKey=0tBiK8O5t10u6vB15mF++FAJ/Ya+HRW/Vs7e+eHh5JM=";
...
static IOTHUBMESSAGE_DISPOSITION_RESULT receive_msg_callback(IOTHUB_MESSAGE_HANDLE message, void* user_context)
{
	...
	TCP_UART_DBG("receive_msg_callback\r\n");
    ...
    if (content_type == IOTHUBMESSAGE_BYTEARRAY)
    {
        ...
        {
            TCP_UART_DBG("Failure retrieving byte array message\r\n");
        }
        else
        {
            TCP_UART_DBG("Received Binary message\r\nMessage ID: %s\r\n Correlation ID: %s\r\n Data: <<<%.*s>>> & Size=%d\r\n", messageId, correlationId, (int)buff_len, buff_msg, (int)buff_len);
        }
    }
    else
    {
        ...
        {
            TCP_UART_DBG("Received String Message\r\nMessage ID: %s\r\n Correlation ID: %s\r\n Data: <<<%s>>>\r\n", messageId, correlationId, string_msg);
        }
    }
...
static int device_method_callback(const char* method_name, const unsigned char* payload, size_t size, unsigned char** response, size_t* resp_size, void* userContextCallback)
{
    ...
    TCP_UART_DBG("Device Method name:    %s\r\n", method_name);
    TCP_UART_DBG("Device Method payload: %.*s\r\n", (int)size, (const char*)payload);
...
static void send_confirm_callback(IOTHUB_CLIENT_CONFIRMATION_RESULT result, void* userContextCallback)
{
    ...
    //TCP_UART_DBG("Confirmation callback received for message %lu with result %s\r\n", (unsigned long)g_message_count_send_confirmations, ENUM_TO_STRING(IOTHUB_CLIENT_CONFIRMATION_RESULT, result));
    TCP_UART_DBG("Confirmation callback received for message  with result \r\n");
}
...
int azure_iothub_Task(void)
{
    IOTHUB_CLIENT_TRANSPORT_PROVIDER protocol;

    IOTHUB_MESSAGE_HANDLE message_handle;
    int telemetry_temperature;
    int telemetry_humidity;
    ...
    TCP_UART_DBG("Creating IoTHub handle\r\n");
    // Create the iothub handle here
    device_handle = IoTHubDeviceClient_CreateFromConnectionString(connectionString, protocol);
    if (device_handle == NULL)
    {
        TCP_UART_DBG("Failure createing Iothub device.  Hint: Check you connection string.\r\n");		
    }
    else
    {
        ...
        char* reportedProperties = "{\"telemetryInterval\":10}";
        (void)IoTHubDeviceClient_SendReportedState(device_handle, (const unsigned char*)reportedProperties, strlen(reportedProperties), reportedStateCallback, NULL);
        ...
        if (proxy_host)
        {
            ...
            {
                TCP_UART_DBG("failure to set proxy\n");
            }
        }		
        ...
        while(g_continueRunning)
        {
            ...
            telemetry_temperature = rand() ;
            telemetry_humidity = rand();

            sprintf(telemetry_msg_buffer, "{\"temperature\":%d,\"humidity\":%d,\"scale\":\"%s\"}", 
			...
			TCP_UART_DBG("Sending message  to IoTHub:%s \n",telemetry_msg_buffer);
