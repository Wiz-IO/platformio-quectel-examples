...
/////////////////////////
static DIGITALTWIN_DEVICE_CLIENT_HANDLE
bg95_DeviceClient_Create(const char* connectionString, _Bool iothubTracingEnabled)
{
    _Bool succeeded = false;
    IOTHUB_DEVICE_HANDLE deviceHandle = NULL;
    DIGITALTWIN_DEVICE_CLIENT_HANDLE deviceClientHandle = NULL;
    DIGITALTWIN_CLIENT_RESULT result;

    deviceHandle = IoTHubDeviceClient_CreateFromConnectionString(connectionString, MQTT_Protocol);
    if (deviceHandle == NULL)
    {
        LogError("Failed to connect using connection string");
        goto done;
    }

    (void)IoTHubDeviceClient_SetOption(deviceHandle, OPTION_LOG_TRACE, &iothubTracingEnabled);

    result = DigitalTwin_DeviceClient_CreateFromDeviceHandle(deviceHandle, &deviceClientHandle);
    if (result != DIGITALTWIN_CLIENT_OK)
    {
        LogError(
            "DigitalTwin_DeviceClient_CreateFromDeviceHandle failed, error: %d, %s",
            result,
            MU_ENUM_TO_STRING(DIGITALTWIN_CLIENT_RESULT, result));
        goto done;
    }

    // On succeeded, the digital twin API owns the deviceHandle, so it should NOT be destroyed.
    deviceHandle = NULL;
    ...
    if (!succeeded)
    {
        //ADUC_DeviceClient_Destroy(deviceClientHandle);
        deviceClientHandle = NULL;
...
DIGITALTWIN_CLIENT_RESULT DigitalTwinClientHelper_RegisterInterfacesAndWait(DIGITALTWIN_DEVICE_CLIENT_LL_HANDLE digitalTwinDeviceHandle, const char* deviceCapabilityModelUri, DIGITALTWIN_INTERFACE_CLIENT_HANDLE* interfaceClientHandles, int numInterfaceClientHandles)
{
    ...
    LogInfo("digitalTwinDeviceHandle=%d\r\n",digitalTwinDeviceHandle);
...
void DigitalTwinClientHelper_Check(DIGITALTWIN_DEVICE_CLIENT_LL_HANDLE digitalTwinDeviceClientHandle)
{
