...
static TX_THREAD* ftp_thread_handle;
...
static unsigned char ftp_stack[THREAD_STACK_SIZE];
...
/*===========================================================================
                               FUNCTION
===========================================================================*/
static void ftp_uart_rx_cb(uint32_t num_bytes, void *cb_data)
{
	if(num_bytes == 0)
	{
...
static void ftp_uart_tx_cb(uint32_t num_bytes, void *cb_data)
{
...
void ftp_uart_dbg_init(void)
{
	...
	uart_cfg.rx_CB_ISR			= (qapi_UART_Callback_Fn_t)&ftp_uart_rx_cb;
	uart_cfg.tx_CB_ISR			= (qapi_UART_Callback_Fn_t)&ftp_uart_tx_cb;
...
void qt_ftp_init_device_info(void)
{
	...
	if(status != QAPI_OK)
	{
		FTP_UART_DBG( "~ qapi_Device_Info_Init fail [%d]\n", status);
	}
	else
	{
		FTP_UART_DBG( "~ qapi_Device_Info_Init OK [%d]\n", status);
	}
}

void quec_ftp_thread(ULONG param)
{
	int ret = 0;
	ULONG dss_event = 0;
	int32 sig_mask;
...
#ifdef QAPI_TXM_MODULE
	if (TX_SUCCESS != txm_module_object_allocate((VOID *)&ftp_thread_handle, sizeof(TX_THREAD))) 
	{
		return -1;
	}
#endif
	ret = tx_thread_create(ftp_thread_handle, "FTP Thread", quec_ftp_thread, NULL,
							ftp_stack, THREAD_STACK_SIZE, THREAD_PRIORITY, THREAD_PRIORITY, TX_NO_TIME_SLICE, TX_AUTO_START);
	if (ret != TX_SUCCESS)
	{
		IOT_INFO("Thread creation failed\n");
		return ret;
